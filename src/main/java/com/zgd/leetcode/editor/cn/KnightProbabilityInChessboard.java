...
public class KnightProbabilityInChessboard {
  ...
  class Solution {
    ...
    /**
     * 动态规划法
     * 动态规划, 规则就是数组保存每一步的数据, 根据上次的数据推导下次的数据. 他有几个特征:
     * 1. 肯定知道一个明确的初始值(比如这题的 移动0步概率是1), 并在此基础上向外扩展, 并每次把扩展的值保存起来(数组)继续扩展下一层.
     * 2. 每层扩展的时候, 参数是固定的, 来源可能是多个上一层的已知数. 比如这题,每层扩展,参数是固定的(坐标), 上一层可能多个(上一步k-1的坐标)
     * <p>
